...
#pragma endregion
		...
		menu_choice = input_validation::cinum(msg_invite, msg_error, 1, 1, 1, 7);
		...
		case 1:
		{
			...
			input_validation::val_char_lenght(SE[SE_amount].Name, 24);
			std::cout << "input company:" << std::endl;
			input_validation::val_char_lenght(SE[SE_amount].Company, 24);
			char msg_invite2[] = "input work expirience(in days):";
			SE[SE_amount].WE = input_validation::cinum(msg_invite2, msg_error, 1, 1, 0);
			std::cin.get();
			std::cout << "input operating system:" << std::endl;
			input_validation::val_char_lenght(SE[SE_amount].software.OS, 12);
			std::cout << "input programming language:" << std::endl;
			input_validation::val_char_lenght(SE[SE_amount].software.CodeLang, 8);
			std::cout << "input database:" << std::endl;
			input_validation::val_char_lenght(SE[SE_amount].software.DB, 8);
		...
		{
			std::system("cls");
			SE_list_manip::print_heading();
			for (int i = 0; i < SE_amount; i++) {
				SE_list_manip::print_SE(SE, i);
			}
			SE_list_manip::print_ending();
			break;
		}
		case 3:
		{
			...
			chosen_WE = input_validation::cinum(msg_invite1, msg_error, 1, 1, 0);
			SE_list_manip::print_heading();
			for (int i = 0; i < SE_amount; i++) {
		...
		case 4:
		{
			SE_list_manip::SE_list_sort(SE, SE_amount);
			std::system("cls");
			SE_list_manip::print_heading();
			bool next = true;
		...
		case 5:
		{
			...
			input_validation::val_char_lenght(chosen_Name, 24);
			SE_list_manip::print_heading();
			for (int i = 0; i < SE_amount; i++) {
				bool chosen = true;
